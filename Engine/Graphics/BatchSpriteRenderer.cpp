...
void BatchSpriteRenderer::Init2()
{
	...
	m_IBO->Generate(Indices, RENDERER_INDICES_SIZE, GL_STATIC_DRAW);
...
void BatchSpriteRenderer::Submit(LRenderableComponent* renderable)
{
	LSpriteComponent* sprite = static_cast<LSpriteComponent*>(renderable);
	if (sprite == nullptr)
	{
...
void BatchSpriteRenderer::Submit(LSpriteComponent& sprite)
{
	const Vec2& Position = sprite.GetLocation();
	const Vec4& Color = sprite.GetTint();
	const Vec2& Size = sprite.GetSize();
...
void BatchSpriteRenderer::Submit2(LSpriteComponent& sprite)
{
	const Vec2& Position = sprite.GetLocation();
	const Vec4& Color = sprite.GetTint();
	const Vec2& Size = sprite.GetSize();
