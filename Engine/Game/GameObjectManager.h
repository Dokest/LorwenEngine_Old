...
public:
	...
	std::vector<class GameObject*> m_GameObjectManager;
	...
	static T* RegisterGameObject(const char* Name)
	{
		...
		Singleton_ObjectManager->m_GameObjectManager.push_back(temp);
	...
	static T* RegisterComponent(class GameObject* owner)
	{
		...
		if (LRenderableComponent* rendComp = dynamic_cast<LRenderableComponent*>(newComponent))
		{
			BaseLevel::CurrentLevel->SubmitNextFrame(rendComp);
		}
