...
// 	void GameInputManager::RegisterInputComponent(InputComponent* inputComponent, GameObject* owner);
//	std::vector<SInput> GameInputManager::m_PossibleInputs;
	...
	unsigned int GameInputManager::BindAxis(class InputComponent& newInput, const char* inputName)
	{
		...
		for (SInputMolecule<AXIS>& axis : m_PossibleAxis)
		{
			...
			{
				axis.Listeners.push_back(&newInput); // THIS DOESNT HAVE TO HAVE A STATE
			}
		}
